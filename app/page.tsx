...
const SpeedChart = dynamic(() => import("@/components/SpeedChart"), {
  ssr: false,
});
const TestHistory = dynamic(() => import("@/components/TestHistory"), {
  ssr: false,
});
...
function Stat({
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 10, fontWeight: 500, color: "var(--subtle)" }}>
        {unit}
      </span>
      <span
        style={{
...
export default function Home() {
  const { state, startTest, history, removeResult, clearAll, isOnline } = useSpeedTest();
  const isRunning = ["ping", "download", "upload"].includes(state.status);
  const showChart =
    state.downloadData.length > 0 || state.uploadData.length > 0;
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      >
        ...
        >
          ...
          <p style={{ margin: 0, fontSize: 14, color: isOnline ? "var(--muted)" : "#ef4444" }}>
            {isOnline
        ...
        >
          ...
          <Stat
            ...
            value={
              state.downloadSpeed !== null
                ? Math.round(state.downloadSpeed * 10) / 10
                : null
            }
            unit="Mbps"
            color="var(--down)"
          ...
          <Stat
            label="Upload"
            value={
              state.uploadSpeed !== null
                ? Math.round(state.uploadSpeed * 10) / 10
                : null
            }
            unit="Mbps"
            color="var(--up)"
          ...
          >
            <div
              style={{
                ...
                background:
                  state.status === "download" ? "var(--down)" : "var(--up)",
                borderRadius: 99,
